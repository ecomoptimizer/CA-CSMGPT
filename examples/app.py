...
system_message = PromptTemplate(
    input_variables = ['topic', 'article', 'target_audience', 'branded_hashtag', 'final_url', 'intent', 'user_input', 'result'],
...
chat = ChatOpenAI(model_name="gpt-3.5-turbo-16k-0613", temperature=0.3, max_tokens=6000)
...
def handle_option_1(text, user_info, post_labels_history):
    ...
    Your task is to analyze the content in {text} as the context and {user_info} and create a list of 10 high-value lead magnets related to the {topic}, enticing our {target_audience} to read the original article.\n
    Great lead magnets are arguably one of the most important parts of any business that wants to start generating leads on autopilot. \n
    Examples of a great lead magnet might be an ebook, a detailed guide, a mind map, access to high-level videos, podcasts, a webinar, coaching, a worksheet, a helpful guide, access to a gated item, a free trial to something. a how-to guide etc...\n
    Ensure each lead magnet is appealing and aligns with the reader's {intent}.\n

    Identify the specific lead magnert type \n
    Identify the core value that each lead magnet represents to our {target_audience} and use the core value to create a clear and concise marketing hook no more than 10 words to use as the lead magnet description.\n
    Finally, for each lead magnet in the list, follow the {format} and label each beginning with LM + 1 and copy each label to the list {post_labels_history}.\n
    After the results are printed to the output window, the user may sned you a {query} to revise the output.\n
    ...
    ''', input_variables=["text", 'user_info', 'format', 'topic', 'target_audience', 'intent', 'result', 'query', 'user_input', 'post_labels_history', ])
    llm_chain = LLMChain(llm=llm, prompt=lead_magnet_template )
    input_data = {"text": text, "format": format, "user_info": user_info, "topic": user_info["topic"], "target_audience": user_info["target_audience"], "intent": user_info["intent"], "user_input": "", "result": "", "query": "",  "post_labels_history": post_labels_history}
...
def handle_option_2(posts, user_info, post_labels_history):
    ...
    [TASK] Your task will utilize {posts} and {user_info}, which contains additional context - {topic}, {intent}, {target_audience}, {branded_hashtag}, and {final_url}.\n
    ...
    7.  Add a minimum of one relevant emoji and every tweet in the thread should contain the {branded_hashtag} and exactly two other unique relevant hashtags.\n
    8.  For each tweet in a thread, provide an ai prompt that is a comprehensive description of the most suitable image to use, in proper format for an ai image generator.   Include the optimal caption and alt tag.\n
    9.  Following {format}, for each thread in the list, assign a label following the designated naming structure identified below. Copy only each label used into {post_labels_history}\n
    10. Speak in a visceral, emoptionally charged voice.\n
    11,  Once the results have been printed to the output window, the user can chat {query} you to ask for revisions.
    ...
    Social-AI:{result}\n\n
    Labels:{post_labels_history}\n''', 
    input_variables=["posts", 'user_info', 'format', 'topic', 'target_audience', 'intent', 'branded_hashtag', 'final_url', 'result', 'query', 'user_input', 'post_labels_history', ])
    llm_chain = LLMChain(llm=llm, prompt=twitter_thread_template)
    input_data = {"posts": posts, "format": format, "user_info": user_info, "topic": user_info["topic"], "target_audience": user_info["target_audience"], "intent": user_info["intent"], "branded_hashtag": user_info["branded_hashtag"], "final_url": user_info["final_url"], "user_input": "", "result": "", "query": "",  "post_labels_history": post_labels_history}
    print("Input data:", input_data)
    result = llm_chain.run(input_data)
    print("LLM output:", result)
    return result, post_labels_history

def handle_option_3(user_info, post_labels_history, post_summaries):
    ...
    Create 3-5 engaging LinkedIn Posts each containing 3 to 5 paragraphs of 2 to 3 sentences related to the {topic} and {intent}, enticing our {target_audience} to read the original article. \n
    ...
    6. Research and include 3 hashtags with each post, {branded_hashtag} and exactly 2 relevant hashtags for LinkedIn.
    7. For each post, include a descriptive AI prompt that will generate the best image to post with each post, including an optimized alt tag and motivating caption.
    8. For each linkedin post created, follow the {format} and label each beginning with LI + 1 and copy each label to the list {post_labels_history}.\n [/TASK]
       ...
       Reslts Format:
              ...
              Content: \n  
              Hashtags: \n 
              AI_Image_prompt, caption and alt tag: 
              \n -----------\n\n
              LI2: \n 
              Content: \n 
              Hashtags:  \n 
              Image_prompt, caption and alt tag: 
              \n -----------\n\n
    ...
    Current conversation:
     -----------
  	...
  	Social-AI:{result}
    Labels:{post_labels_history}''', 
    input_variables=["post_summaries", 'user_info', 'format', 'topic', 'target_audience', 'intent', 'branded_hashtag', 'final_url', 'result', 'query', 'user_input', 'post_labels_history'])
    llm_chain = LLMChain(llm=llm, prompt=linkedin_post_template)
    input_data = {"post_summaries": post_summaries, "format": format, "user_info": user_info, "topic": user_info["topic"], "target_audience": user_info["target_audience"], "intent": user_info["intent"], "branded_hashtag": user_info["branded_hashtag"], "final_url": user_info["final_url"], "user_input": "", "result": "", "query": "",  "post_labels_history": post_labels_history}
    result = llm_chain.run(input_data)
    return result, post_labels_history
...
def handle_option_4(post_summaries, user_info, post_labels_history):
    ...
    in this template explicitly. [Social-AI], As a Copywriter & Facebook Content Expert at the All-In-One Social Media Content Suite, your goal is to analyze the provided information in {post_summaries} and {user_info} which includes -  {topic}, {intent}, {target_audience}, {branded_hashtag}, and {final_url}.
    ...
    Your task is to create a minimum of 3 to 5 posts each containing 3 to 5 paragraphs of 2 to 3 sentences each related to the {topic} and {intent}, enticing our {target_audience} to read the original article.   Get your content from {post_summaries} includes summarized posts from the main article separated by the headers.
    ...
    Conclusion
    Hastags
    AI Image Prompt, Optimized caption and alt tag
...
def handle_option_5(posts, user_info, post_labels_history):
    ...
    Use the important points made in the {posts} about the {topic} for your posts.\n

    For this prompt, you will review the {posts} and {user_info}. Then create at least 5 to 10 Instagram posts that cover the most important points made in the {posts} that our
    ...
    6. for each Instagram post created, follow the {format} and label each beginning with Inst + 1 and copy each label to the list {post_labels_history}.\n
    After the results are printed to the output window, the user may sned you a {query} to revise the output.
    Present the results in the Following format:
	Inst1
    ...
    Current conversation:
  	...
  	Social-AI:{result}
    Labels:{post_labels_history}''', 
    input_variables=["posts", 'user_info', 'format', 'topic', 'target_audience', 'intent', 'branded_hashtag', 'result', 'query', 'user_input', 'post_labels_history'])
    llm_chain = LLMChain(llm=llm, prompt=instagram_post_template)
    input_data = {"posts": posts, "format": format, "user_info": user_info, "topic": user_info["topic"], "target_audience": user_info["target_audience"], "intent": user_info["intent"], "branded_hashtag": user_info["branded_hashtag"], "user_input": "", "result": "", "query": "",  "post_labels_history": post_labels_history}
    result = llm_chain.run(input_data)
    return result, post_labels_history

def handle_option_6(posts, user_info, post_labels_history):
    pinterest_post_template = PromptTemplate(template= '''{posts} [Social-AI], You are a world class journalist & the Pinterest Content Expert at the All In One Social Media Content Suite.  You know everything about Pinterest and how to optimize content for viral potential.  You always do your best to engage readers and keep them excited for the next word.
    You are known for being articulate.  And have as reputation for creating targeted engaging content.  Use the {posts} list of important points made in the article as your topics.

    For this prompt, your task is to review the {posts} and {user_info}.  Then Create at least 5 to 10 pins that cover the most important points made in the {posts} about the {topic} that our {target_audience} has {intent} for and would engage with.
    The most important part of a Pinterest pin begins with the image creation.  For each pin, create the most intriquing ai image prompt possible that shows the essence of the pin.\n 

    Objectives:
    1. Pinterest posts must start with creating the best relevant inage to build around the post topic. \n
    Create the most intriquing ai image prompt possible that shows the essence of the pin.  Describe in detail a 2 to 5 word text overlay.\n
    2. Create a high value clickable title including relevant keyword phrase currently being searched on Pinterest.\n
    3. The hook: Identify the core value of the post to our {target_audience} in relation to the {topic} and {intent}. Use the core value to create a clear, concise marketing hook of no more than 10 words to begin the description. \n
    ...
    Current conversation:
  	...
  	Human: {user_input}
  	Social-AI:{result}
    Labels:{post_labels_history}''', input_variables=['posts', 'user_info', 'format', 'topic', 'target_audience', 'intent', 'branded_hashtag', 'final_url', 'result', 'query', 'user_input', 'post_labels_history'])
...
def handle_option_7(posts, user_info, post_labels_history):
    tiktok_post_template = PromptTemplate(template='''{posts} [Social-AI], You are a world class journalist & the TikTok Content Expert at the All In One Social Media Content Suite.  \n
    ...
    You are known for being articulate and have as reputation for creating targeted engaging content.   For this prompt, you will review the {posts} and {user_info} about the {topic} and then

    Your task is to create 6 -10 relevant video transcripts and hooks for short TikTok videos based on the most important points made in the {posts} relevant to the {target_audience} and the {intent}.
...
def handle_option_8(post_summaries, user_info, post_labels_history):
    ...
    9. Include up to 30 optimized relevant tags to add to each video. \n 
  	10.Follow the {format} and Label each YouTube video YT1, YT2, YT3 etc.. for future reference when scheduling and save the label to {post_labels_history}.[/Task] \n
    After the results are printed to the output window, the user may sned you a {query} to revise the output.
    [COMPETENCE MAPS]
...
def handle_option_9(text, user_info, post_labels_history):
    ...
    Current conversation: \n
  	...
  	Social-AI:{result} \n
    Labels:{post_labels_history}''', 
    input_variables=['text', 'user_info', 'format', 'topic', 'target_audience', 'intent', 'branded_hashtag', 'final_url', 'post_labels_history', 'result', 'query', 'user_input'])
    llm_chain = LLMChain(llm=llm, prompt=mailing_list_template)
    input_data = {"text": text, "format": format, "user_info": user_info,  "topic": user_info["topic"], "target_audience": user_info["target_audience"], "intent": user_info["intent"],  "branded_hashtag": user_info["branded_hashtag"], "final_url": user_info["final_url"], "user_input": "", "result": "", "query": "",  "post_labels_history":post_labels_history,}
    result = llm_chain.run(input_data)
    return result, post_labels_history

def handle_option_10(tesx, user_info, post_labels_history):  
    ...
    input_data = {"text": text, "format": format, "user_info": user_info,  "topic": "topic", "target_audience": user_info["target_audience"], "intent": user_info["intent"], "branded_hashtag": user_info["branded_hashtag"], "final_url": user_info["final_url"], "user_input": "", "result": "", "query": "",  "post_labels_history": post_labels_history}
    result = llm_chain.run(input_data)
    return result, post_labels_history
...
def handle_option_12(user_info, post_labels_history):
    ...
    {text}
  	{post_labels_history}
  	Human: {user_input}
  	Social-AI:{result}''', input_variables=["post_labels_history", 'text', 'user_info', "topic", 'result', 'query', 'user_input',])
    llm_chain = LLMChain(llm=llm, prompt=scheduler_template)
    input_data = {"post_labels_history": post_labels_history, "text": text, "user_info": user_info,  "topic": user_info["topic"],  "user_input": "", "result": ""}
    result = llm_chain.run(input_data)
    return result
